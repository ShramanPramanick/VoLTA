...
parser = argparse.ArgumentParser(description='Train Multimodal BT with GOT')
...
parser.add_argument('--data_root_coco', default='/checkpoint/ljng/data/COCO/coco2014/', type=str, help='Root directory of the mscoco data')
parser.add_argument('--data_root_cc', default='/checkpoint/ljng/data/cc3m/cc3m/', type=str, help='Root directory of the cc data')
parser.add_argument('--data_root_sbu', default='/checkpoint/ljng/data/SBU/images/', type=str, help='Root directory of the sbu data')
parser.add_argument('--data_root_vg', default='/datasets01/VisualGenome1.2/061517/', type=str, help='Root directory of the vg data')
...
parser.add_argument('--sbu', action="store_true")
...
def adjust_learning_rate(args, optimizer, loader, step):
    ...
    warmup_steps = int(max_steps/50)      ### 2 * len(loader), In originalBT repo, this constant is 10
...
def main_worker(gpu, args):
    ...
    if args.rank == 0:
        print("COCO dataset:", len(image_list_coco))
    
    image_list_cc, text_list_cc = get_id_list_separate(args.data_root_cc) if args.cc else ([], [])
    if args.rank == 0:
        print("CC3M dataset:", len(image_list_cc))
    
    image_list_sbu, text_list_sbu = get_id_list_separate(args.data_root_sbu) if args.sbu else ([], [])
    if args.rank == 0:
        print("SBU dataset:", len(image_list_sbu))
    
    image_list_vg, text_list_vg = get_id_list_vg(args.data_root_vg) if args.vg else ([], [])
    if args.rank == 0:
        print("VG dataset:", len(image_list_vg))
    
    image_list = image_list_coco + image_list_cc + image_list_sbu + image_list_vg
    text_list = text_list_coco + text_list_cc + text_list_sbu + text_list_vg
    if args.rank == 0:
        print("Total dataset:", len(image_list))
    ...
    train_loader = DataLoader(cc_data, batch_size=per_device_batch_size, num_workers=args.num_workers, sampler=sampler, pin_memory=True, drop_last=True)
    ...
    # training loop
    results = {'BT_train_loss': [], 'GOT_train_loss': [], 'MLM_train_loss': [], 'ITM_train_loss': []} # , 'test_acc@1': [], 'test_acc@5': []}
    ...
    #torch.autograd.set_detect_anomaly(True) ## Do this use this
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for step, (image, token, mask, mlm_ids, mlm_labels) in enumerate(train_loader, start=epoch * len(train_loader)):
            ...
            #pos_false_im1, pos_false_im2 = false_image[0], false_image[1]
            ...
            #print('Step ' + str(len(results['train_loss']) + 1) + ': LR ', train_optimizer.param_groups[0]["lr"])
