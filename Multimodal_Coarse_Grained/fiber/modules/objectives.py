...
def compute_itc_recall(pl_module):
    ...
    if rank == 0:
        torch.save(tiids,'coco_itc_tiids.pt')
        torch.save(iids,'coco_itc_iids.pt')
        torch.save(scores,'coco_itc_scores.pt')
...
def compute_itm_recall(pl_module):
    ...
    for img_batch in tqdm.tqdm(image_preload, desc="rank loop"):
        ...
        for txt_batch in text_preload:
            ...
            with torch.cuda.amp.autocast():
                score = F.softmax(pl_module.itm_score(
                    pl_module.infer(
                        {
                            "text_ids": txt_batch["text_ids"],
                            "text_masks": txt_batch["text_masks"],
    ...
    if rank == 0:
        torch.save(tiids,'coco_itm_tiids.pt')
        torch.save(iids,'coco_itm_iids.pt')
        torch.save(scores,'coco_itm_scores.pt')
